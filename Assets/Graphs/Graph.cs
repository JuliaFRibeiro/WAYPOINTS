...
public class Graph
{
    ...
    public Graph() { }
    public void AddNode(GameObject id, bool removeRenderer = true, bool
    removeCollider = true, bool removeId = true)
    {
        ...
        if (removeId)
        {
            TextMesh[] textms = id.GetComponentsInChildren<TextMesh>() as
            TextMesh[];
        ...
        while (open.Count > 0)
        {
            ...
            foreach (Edge e in thisnode.edgelist)
            {
                ...
                if (closed.IndexOf(neighbour) > -1)
                    continue;
                tentative_g_score = thisnode.g + distance(thisnode,
                neighbour);
    ...
    public void debugDraw()
    {
        ...
        {
            Debug.DrawLine(edges[i].startNode.id.transform.position,
            edges[i].endNode.id.transform.position, Color.red);
        }
        //draw directions
        for (int i = 0; i < edges.Count; i++)
        {
            Vector3 to = (edges[i].startNode.id.transform.position -
            edges[i].endNode.id.transform.position) * 0.05f;
